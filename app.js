...
function printWorksheetSummary(formId) {
  ...
  const memberVisual = `<span class="person-initials">${initials(member.name)}</span>`;
  const spouseVisual = member.profile.spouseName
  ...
  report.document.write(`<!doctype html><html><head><title>F.I.T. Summary - ${escapeHtml(form.assignedName || member.name)}</title>
    ...
    <div class="people"><div class="person">${memberVisual}<div><strong>${escapeHtml(member.name)}</strong><br><span>Account holder</span></div></div>${member.profile.spouseName ? `<div class="person">${spouseVisual}<div><strong>${escapeHtml(member.profile.spouseName)}</strong><br><span>Spouse</span></div></div>` : ""}</div>
